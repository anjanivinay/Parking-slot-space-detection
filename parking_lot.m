...
folder = pwd;
fullFileName = 'Parking Lot without Cars.jpg';
%fullFileName = fullfile(folder, fullFileName);
% Check if file exists.
if ~exist(fullFileName, 'file')
	% The file doesn't exist -- didn't find it there in that folder.
	% Check the entire search path (other folders) for the file by stripping off the folder.
	fullFileNameOnSearchPath = fullFileName; % No path this time.
...
axis('on', 'image');
caption = sprintf('Reference Image : "%s"', fullFileName);
...
folder = pwd;
fullFileName = 'Parking Lot with Cars.jpg';
%fullFileName = fullfile(folder, fullFileName);
% Check if file exists.
if ~exist(fullFileName, 'file')
	% The file doesn't exist -- didn't find it there in that folder.
...
imshow(rgbTestImage, []);
axis('on', 'image');
caption = sprintf('Test Image : "%s"', fullFileName);
title(caption, 'FontSize', fontSize, 'Interpreter', 'None');
drawnow;
hp = impixelinfo(); % Set up status line to see values when you mouse over the image.
...
folder = pwd;
fullFileName = 'Parking Lot Mask.png';
%fullFileName = fullfile(folder, fullFileName);
% Check if file exists.
if ~exist(fullFileName, 'file')
	% The file doesn't exist -- didn't find it there in that folder.
...
imshow(mask, []);
axis('on', 'image');
caption = sprintf('Mask Image : "%s"', fullFileName);
title(caption, 'FontSize', fontSize, 'Interpreter', 'None');
drawnow;
hp = impixelinfo(); % Set up status line to see values when you mouse over the image.
...
for k = 1 : length(props)
	...
	if percentageFilled(k) > 0.10
		...
		plot(x, y, 'r.', 'MarkerSize', 30, 'LineWidth', 4);
